...
// Set up URL callback listener to detect and process OAuth callback URLs
...
async function handleGoogleAuth() {
  try {
    logger.time('Starting Google auth flow...');
    ...
    return new Promise((resolve, reject) => {
      chrome.identity.getAuthToken({ interactive: true }, async (token) => {
        ...
        try {
          logger.time('Got auth token, processing...');
...
chrome.action.onClicked.addListener((tab) => {
  ...
  try {
    ...
    if (supported) {
      ...
      // Not on a supported site, open the gallery directly
      logger.log('Not on a supported site
